...
Deployed Application: [Student Store Deployed Site](https://student-store-frontend-w141.onrender.com/)
...
The labs gave me a solid foundation for the core work — defining Prisma models, writing Express routes, and structuring the API into models, controllers, and routes. Where I felt less prepared was the transactional logic for `POST /orders` (creating an order and all of its order items in a single Prisma `$transaction` while computing prices server-side) and the deployment step on Render, especially wiring up environment variables like `DATABASE_URL` and `VITE_API_BASE_URL` so the deployed frontend and backend could talk to each other.

* If you had more time, what would you have done differently? Would you have added additional features? Changed the way your project responded to a particular event, etc.

If I had more time, I would add user authentication so that "Past Orders" could be scoped to the logged-in user instead of showing every order, and I'd surface all invalid product IDs in a single error response instead of failing on the first one. I'd also improve form validation on checkout and add loading/skeleton states across the UI for a smoother experience.

* Reflect on your project demo, what went well? Were there things that maybe didn't go as planned? Did you notice something that your peer did that you would like to try next time?

I demo'd the website pretty well, and showed multiple endpoints. Even when one failed, I still recovered. However, I forgot to show prisma studion and display the database.
