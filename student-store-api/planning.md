...
`GET`    | `/orders/:order_id`    | No body. No query params. | `200 OK` -> order with items: `{ order_id, customer_id, total_price, status, created_at, order_items: [{ order_item_id, product_id, quantity, price, product: { id, name, price, ... } }] }` (each item nests its related `product` so the UI can show product names) | `404` -> `{"error": "Order not found"}`
